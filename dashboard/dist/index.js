...
(function () {
  ...
  function MeshCorePage() {
    ...
    return React.createElement("div", { className: "mc-dashboard" },
      ...
      // ── Admin Query card ────────────────────────────────────────────
      React.createElement(AdminQueryCard, {})
    );
  }

  // ── AdminQueryCard sub-component ──────────────────────────────────────
  function AdminQueryCard() {
    var _React = React;
    var useState = _React.useState;
    var useEffect = _React.useEffect;

    var _a = useState(""), queryNode = _a[0], setQueryNode = _a[1];
    var _b = useState("stats-core"), queryCmd = _b[0], setQueryCmd = _b[1];
    var _c = useState(""), queryPw = _c[0], setQueryPw = _c[1];
    ...
    var COMMANDS = [
      // Status & telemetry (binary protocol — parsed output)
      { value: "stats-core", label: "stats-core — battery, uptime, packets, RSSI, SNR" },
      { value: "stats-radio", label: "stats-radio — RF telemetry (LPP)" },
      { value: "stats-packets", label: "stats-packets — packet telemetry (LPP)" },
      { value: "ver", label: "ver — firmware version & board" },
      { value: "board", label: "board — board name" },
      // Network
      { value: "neighbors", label: "neighbors — visible nodes (zero-hop)" },
      { value: "clock", label: "clock — remote node time" },
      // Info queries
      { value: "get owner.info", label: "get owner.info — owner details" },
      ...
      { value: "req_status", label: "req_status — raw status request" },
      { value: "req_neighbours", label: "req_neighbours — raw neighbour request" },
      { value: "req_telemetry", label: "req_telemetry — raw telemetry request" },
      { value: "req_acl", label: "req_acl — access control list" },
      { value: "req_owner", label: "req_owner — owner information" },
      { value: "req_clock", label: "req_clock — remote clock" },
      { value: "req_regions", label: "req_regions — regions list" },
    ];
